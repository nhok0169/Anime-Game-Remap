...
promotes the baseline below. Current baseline: **16 warnings and 0 errors** (re-measured
2026-09-17, four ways: with and without `-W`, and with and without the `.pyi` stand-ins --- all four
report the same 16, so the **26** an earlier revision of this line recorded is stale, most likely by the
`apiExamples.rst` regeneration of 2026-09-17). Sphinx's own closing line (`build succeeded, 16
warnings.`) is the number to quote; grepping for `WARNING:` agrees with it here but a multi-line warning
would not. The 16 are 6 duplicate `tutorial.rst` labels, 3 undefined labels there, 1 malformed
enumerated list in `findVertexGroupRemap.rst`, **4 `duplicate object description` warnings from the
...
## THE PUBLISHED SITE COMPILES NOTHING, SO `core.pyi` AND `core/xml` *ARE* THE DOCS (2026-09-17)

The site is <https://anime-game-remap.readthedocs.io>, built from `.readthedocs.yaml` at the repo root
--- the only Read the Docs file in the tree. All that runner does is install `Docs/requirements.txt`
and run `Docs/src/conf.py`: **no submodules, no CMake, no Doxygen, and no pip install of the API.** It
cannot do otherwise. Installing the API runs scikit-build-core, which runs CMake, which does
`find_package(Z3 CONFIG REQUIRED)`; z3 alone is ~45 minutes from cold, against a build time limit.

So the compiled half of the library is documented there out of what the repo already tracks:

| what renders | from | regenerated by |
| --- | --- | --- |
| `coreAPI.rst`'s C++ entities | `api/src/cpp/core/xml` (Doxygen **1.17.0**) | `Tools/APIBuilder/main.py -d` |
| `api.rst`'s compiled-backed classes (155 of its 182) | `FixRaidenBoss2/core.pyi` (pybind11-stubgen against pybind11 **3.0.4**) | `Tools/APIBuilder/main.py -d` --- the same flag |
| its 4 Cython classes | `api/src/cy/src/tools/*.pyx` --- the source itself | nothing to regenerate |

**The consequence is the part to remember: a pybind11 class you add without regenerating `core.pyi`
renders on YOUR machine and is missing from the published site.** Locally autodoc imports the real
`.pyd` and sees everything; on Read the Docs it imports the stub. It cuts the other way too --- a
stale `core.pyi` publishes a signature the library no longer has, and a stale `core/xml` publishes a
C++ class that has been renamed. Checking `git status` on those two artifacts is part of finishing a
docs change, not housekeeping. (It also means the published `api.rst` can be *ahead* of a maintainer's
Windows build: the three `try: from .core import ...` blocks at the bottom of
`FixRaidenBoss2/__init__.py` are guarded precisely because a stale `.pyd` lacks those names, and the
stub always has them.)

`Docs/src/extensions/compiledStubs.py` is what serves those artifacts to `autodoc` and
`attributetable`, which both *import* the module they document. It registers a `sys.meta_path` finder
for each extension module that is **not** built beside its stub, and steps aside entirely for one that
is --- so a maintainer's build is byte-for-byte the build it always was, and nothing about it is
conditional on being on Read the Docs.

Four things had to be repaired in the generated `.pyi` to make it importable at all, and every one of
them is printed by the build (`[compiledStubs] ...` lines, near the top of the log):

1. **A required argument after a defaulted one.** `PyModMappedAssets.cpp`'s `replace` binding lists
   `py::arg("toAssetName")` with no default after three defaulted args --- legal pybind11, illegal
   Python --- so `core.pyi` is a file python *refuses to parse*. This is a real defect in the binding,
   not a docs problem: any type checker reading the shipped stub hits it too. The docs work around it
   by giving the trailing parameter a default.
2. **Classes written in alphabetical order.** A base class is the one part of a `class` statement
   python evaluates immediately, so `class BaseIniFixer(CppBaseIniFixer)` fails on the letter B. 25 of
   the 210 classes inherit from one written later in the file; they are topologically reordered.
3. **Method bodies that return nothing.** The API's own data modules *call* the core while they are
   being imported (`HashData = _CppHashes().repo.toNestedDict()`), so every stubbed method is made to
   return a stand-in value instead of `None`.
4. **Enum members written as bare annotations.** `Amber: typing.ClassVar[ModTypeId]` defines no
   attribute at all, and `VertexCountData` is keyed by `ModTypeId.Amber`. Each gets the value from its
   own trailing `# value = <ModTypeId.Amber: 0>` comment, so the rendered page reads the same as one
   built against the real module.

The Cython stubs are a different story and are **not** used. `buildDocs()` runs `pybind11_stubgen`
over *every* installed module, Cython ones included, and what comes back for those is useless:
`CyDictTools.pyi` re-imports the extension it is supposed to describe (`from CyDictTools import
CyDictTools`), declares no class, and is not even valid Python (its `from __future__` import sits on
line 3). They are tracked, so don't read one and conclude the Cython layer has no docs. The `.pyx` sources carry the
real numpydoc docstrings, so `compiledStubs.py` translates those instead --- classes, methods,
docstrings, and nothing else --- and finds them by the `# distutils: name =` line rather than by
guessing that `DictTools.pyx` builds `CyDictTools`.

**Measured both ways on 2026-09-17**, over the same source tree --- one build importing the real
extension modules, one with them blocked outright and `AGREMAP_DOCS_STUBS=force`:

| | compiled | from the tracked artifacts |
| --- | --- | --- |
| `api.rst` classes | 182 | **182** |
| `api.rst` members | 1333 | **1330** |
| `coreAPI.rst` C++ entities | 6369 | **6369** |
| warnings / errors | 16 / 0 | **16 / 0** |

The three missing members are `GameTypeId.name`, `ModTypeId.name` and `RegDelimitedAddMode.name`:
pybind11 gives an enum's `name` property a docstring at runtime and the generated stub does not, and
autodoc documents only what has one. That is the entire difference between the published page and a
maintainer's. The same build with the modules blocked and the stand-ins turned OFF dies with
`Extension error (attributetable): No module named 'FixRaidenBoss2.core'`, which is what Read the Docs
would have done --- worth knowing, because it is the only way to be sure a green stub build means
anything (Overview's habit 34).

**Test the published path locally, in one command, without deleting anything:**

```bash
cd Docs
AGREMAP_DOCS_STUBS=force py -3 -m sphinx -b html -E --keep-going src build/html
```

`force` makes the stand-ins be used even though the real modules are right there. If the page you
...
- `build: jobs: pre_build: - doxygen "...core/Doxyfile"` --- unnecessary, because `core/xml` is
  committed, and incapable of working anyway: the Doxyfile's `INPUT = include src` and its empty
  `OUTPUT_DIRECTORY` are relative to the **working directory**, which for a Read the Docs job is the
  checkout root. Doxygen was looking for `./include` and `./src` at the top of the repo and would have
  written `./xml` there rather than into the folder Breathe reads.
