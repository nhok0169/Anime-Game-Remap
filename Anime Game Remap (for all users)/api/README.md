...
| -c, --uncompressTextures | Whether to leave textures uncompressed. <br> <br> Pick your poison, do you want the fix to run faster, but your textures take up more space OR your fix to run slower, but textures take minimal space. <br> <br> This option only turns compression **off**. If it is not specified, each mod type's own texture edits decide for themselves whether to compress what they write. |
