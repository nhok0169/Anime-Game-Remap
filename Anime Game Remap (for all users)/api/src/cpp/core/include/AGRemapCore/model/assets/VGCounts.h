#ifndef AGRemapCore_VGCounts_H
#define AGRemapCore_VGCounts_H
...
namespace AGRemapCore {
     ...
     * @brief
     @rst
     The ``vg_count`` of a wwmi draw slot -- how many bones that component brings, which a mod's slot section hands to ``$\\wwmiv1\\vg_count`` before it merges the skeleton -- a :cpp:class:`ModMappedAssets` pre-populated with this project's real data
     (:cpp:func:`Data::getVGCountDataRows`) :raw-html:`<br />` :raw-html:`<br />`
     ...
     @endrst
     */
    class VGCounts: public ModMappedAssets<std::string, std::string> {
        public:
            /**
             ...
             */
            explicit VGCounts(std::unordered_map<std::string, std::vector<std::string>> map = {});
    };
}
