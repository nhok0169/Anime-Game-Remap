#ifndef AGRemapCore_VGOffsets_H
#define AGRemapCore_VGOffsets_H
...
namespace AGRemapCore {
     ...
     * @brief
     @rst
     The ``vg_offset`` of a wwmi draw slot -- where that component's bones start in wwmi's merged skeleton, which a mod's slot section hands to ``$\\wwmiv1\\vg_offset`` before it merges the skeleton -- a :cpp:class:`ModMappedAssets` pre-populated with this project's real data
     (:cpp:func:`Data::getVGOffsetDataRows`) :raw-html:`<br />` :raw-html:`<br />`
     ...
     @endrst
     */
    class VGOffsets: public ModMappedAssets<std::string, std::string> {
        public:
            /**
             ...
             */
            explicit VGOffsets(std::unordered_map<std::string, std::vector<std::string>> map = {});
    };
}
