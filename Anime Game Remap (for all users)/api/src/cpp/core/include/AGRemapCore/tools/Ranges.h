    ...
    */
    class Ranges {
        public:
            ...
            @endrst
            */
            using Range = std::pair<std::optional<int>, std::optional<int>>;
            ...
            */
            explicit Ranges(const std::vector<int>& values);
            ...
            Equivalent to ``createFromSet(std::set<int>(values.begin(), values.end()))``
            @endrst
            *
            * @param values The values to include
            */
            explicit Ranges(const std::unordered_set<int>& values);
            ...
            * @param values The values to include
            */
            explicit Ranges(const std::set<int>& values);
            ...
            **Default**: ``false``, meaning ``this->ranges`` is assumed to
            ...
            @endrst
            */
            void add(int value, bool normalize = false);
            ...
            **Default**: ``false``, meaning ``this->ranges`` is assumed to
            ...
            @endrst
            */
            void remove(int value, bool normalize = false);
            ...
            */
            static Ranges createFromList(const std::vector<int>& values);
    
            /**
             * @copydoc createFromList(const std::vector<int>&)
             */
            static Ranges createFromSet(const std::set<int>& values);
        ...
        private:
            ...
            */
             ...
             * @brief
             @rst
            Builds a Ranges instance from a sorted list of unique integers,
            merging consecutive runs into contiguous ranges
            @endrst
            *
            * @param sortedUnique A sorted list of unique integers
            *
            * @return A new Ranges instance whose ranges cover exactly the values in ``sortedUnique``
            */
            static Ranges buildFromSortedUnique(const std::vector<int>& sortedUnique);
    };
}
