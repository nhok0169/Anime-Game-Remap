...
namespace AGRemapCore {
    namespace {
        // Built once and shared by every IndexCounts instance -- see Hashes.cpp's identical note for
        ...
        const ModDictAssets<std::string, std::string>& prototypeRepo() {
            static const ModDictAssets<std::string, std::string> repo = []() {
                const std::vector<std::pair<std::vector<std::string>, std::string>>& rawRows = Data::getIndexCountDataRows();
    ...
    IndexCounts::IndexCounts(std::unordered_map<std::string, std::vector<std::string>> map):
