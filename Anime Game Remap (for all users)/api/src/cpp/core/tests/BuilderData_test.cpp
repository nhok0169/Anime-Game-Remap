...
void testTableShape() {
    ...
    check(IniFixBuilderData::repo()->size() == 132,
          "the fix table has 132 rows -- 78 historical, the 50 at 6.1 this port added, and the four WuWa stubs");

    // The remove table has no Python original -- one row per GI mod type, all at 4.0.
    check(IniRemoveBuilderData::repo()->size() == 51, "the remove table has one row per mod type (43 GI, plus Yelan, YelanTranquil, Bennett and BennettAdventure, plus the four WuWa stubs)");
