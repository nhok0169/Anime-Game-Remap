...
//     table is 91: 73 Python rows fanned out to 78, plus the thirteen real 6.1 rows
...
void testTableShape() {
    ...
    check(IniFixBuilderData::repo()->size() == 91,
          "the fix table has 91 rows -- the 73 Python rows fanned out per target mod to 78, plus the thirteen real 6.1 "
          "rows (Raiden, Amber, AmberCN, Mona, MonaCN, Rosaria, RosariaCN, and Jean/JeanCN/JeanSea with TWO "
          "each -- one per target mod, which is what replaced the pure-Python MultiModFixer)");
