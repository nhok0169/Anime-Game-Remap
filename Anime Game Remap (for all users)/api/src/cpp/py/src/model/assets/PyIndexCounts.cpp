...
void initCppIndexCounts(pybind11::module_ &m) {
    py::class_<AGRC::IndexCounts, CoreModMappedAssets, py::smart_holder>(m, "IndexCounts", R"doc(
    ...
    * type
    ...
    :class:`Indices` files a WWMI ``match_first_index``
        ...
        .def(py::init([](const py::object &map) {
            return std::make_unique<AGRC::IndexCounts>(map.is_none() ? PyObjectMap{} : convertMap(map.cast<py::dict>()));
