...
void initCppShapeKeyChecksums(pybind11::module_ &m) {
    py::class_<AGRC::ShapeKeyChecksums, CoreModMappedAssets, py::smart_holder>(m, "ShapeKeyChecksums", R"doc(
    ...
    * type
    ...
    :class:`Indices` files a WWMI ``match_first_index``
        ...
        .def(py::init([](const py::object &map) {
            return std::make_unique<AGRC::ShapeKeyChecksums>(map.is_none() ? PyObjectMap{} : convertMap(map.cast<py::dict>()));
