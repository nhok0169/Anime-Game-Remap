...
void initCppVGCounts(pybind11::module_ &m) {
    py::class_<AGRC::VGCounts, CoreModMappedAssets, py::smart_holder>(m, "VGCounts", R"doc(
    ...
    * type
    ...
    :class:`Indices` files a WWMI ``match_first_index``
        ...
        .def(py::init([](const py::object &map) {
            return std::make_unique<AGRC::VGCounts>(map.is_none() ? PyObjectMap{} : convertMap(map.cast<py::dict>()));
