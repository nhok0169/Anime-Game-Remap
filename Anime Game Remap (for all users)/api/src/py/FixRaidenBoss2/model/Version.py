...
class Version():
    ...
    def _add(self, newVersion: Union[str, float, VersionType]):
    ...
    def findClosest(self, version: Optional[Union[str, float, VersionType]], fromCache: bool = True) -> Optional[VersionType]:
        """
        ...
        result = self.findClosestFromSortedList(self._versions, version)
        self._versionCache[version] = result
        return result
    
    @classmethod
    def findClosestFromSortedList(cls, versions: List[Union[str, float, VersionType]], version: Optional[Union[str, float, VersionType]]) -> Optional[VersionType]:
        """
        Finds the closest version available from a sorted list of versions

        Parameters
        ----------
        versions: List[Union[:class:`str`, :class:`float`, `packaging.version.Version`_]]
            The sorted list of versions to search
        ...
        found, ind = Algo.binarySearch(versions, version, lambda v1, v2: cls.compareVersions(v1, v2))

        result = 0
        if (found):
            result = versions[ind]
        elif (ind > 0):
            result = versions[ind - 1]
        else:
            result = versions[0]
    ...
    @classmethod
    def findClosestFromList(cls, versions: List[Union[str, float, VersionType]], version: Optional[Union[str, float, VersionType]]) -> Optional[VersionType]:
        """
        Finds the closest version available from a list of versions
        ...
        if (version is not None):
            version = cls.getVersion(version)

            compatibleVersions = []
            for currentVersion in versions:
                convertedVersion = cls.getVersion(currentVersion)
                if (convertedVersion <= version):
                    compatibleVersions.append(convertedVersion)

        else:
            compatibleVersions = list(map(lambda currentVersion: cls.getVersion(currentVersion), compatibleVersions))

        if (not compatibleVersions):
            return None
        
        return max(compatibleVersions)
##### EndScript
