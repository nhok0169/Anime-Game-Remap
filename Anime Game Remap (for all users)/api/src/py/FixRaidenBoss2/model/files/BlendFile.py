...
class BlendFile(BufFile):
    ...
    def __init__(self, src: Union[str, bytes], elements: Optional[List[BufElementType]]):
        if (elements is None):
            elements = [BufElementTypes.BlendWeightFloatRGBA.value, BufElementTypes.BlendIndicesIntRGBA.value]

        super().__init__(src, elements, fileType = "Blend.buf")
