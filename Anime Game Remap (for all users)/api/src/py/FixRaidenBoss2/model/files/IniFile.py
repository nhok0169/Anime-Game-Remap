...
class IniFile(File):
    ...
    version: Optional[Union[:class:`str`, :class:`float`, `packaging.version.Version`_]]
        ...
        If this value is ``None``, then will assume the .ini file comes from the latest game version. :raw-html:`<br />` :raw-html:`<br />`

        **Default**: ``None``

    toVersion: Optional[Union[:class:`str`, :class:`float`, `packaging.version.Version`_]]
        The game version we want to fix the .ini file to :raw-html:`<br />` :raw-html:`<br />`

        If this value is ``None``, then will assume to fix the .ini file to the latest game version. :raw-html:`<br />` :raw-html:`<br />`
    ...
    toVersion: Optional[`packaging.version.Version`_]
        The game version we want to fix the .ini file to :raw-html:`<br />` :raw-html:`<br />`

        If this value is ``None``, then will assume to fix the .ini file to the latest game version.
    ...
    def __init__(self, file: Optional[str] = None, logger: Optional["Logger"] = None, txt: str = "", modTypes: Optional[Set[ModType]] = None, defaultModType: Optional[ModType] = None, 
        ...
        self.toVersion = Version.getVersion(toVersion)
        ...
        self.resources: List[IniResource] = []
        self.fileDownloads: List[RemapIniDownload] = []
    ...
    def classify(self, flush: bool = False) -> bool:
        ...
        classifyStats = self._iniClassifier.classify(self._fileLines)

        modType = classifyStats.getModType()
    ...
    def injectAddition(self, addition: str, beforeOriginal: bool = True, keepBackup: bool = True, fixOnly: bool = False, update: bool = False) -> str:
        """
        Adds and writes the text fix to the .ini file
    ...
    def makeRemapIniResource(self, ifTemplate: IfTemplate, makeRes: Callable[[str, "IniFile"], IniResource]) -> List[IniResource]:
        """
        Creates the data needed for a particular ``[Resource.*]`` `section`_ in the original .ini file
        ...
        makeRes: Callable[[:class:`str`, :class:`IniFile`], :class:`IniResource`]
            Function for building the resource once found :raw-html:`<br />` :raw-html:`<br />`

            The function takes in:

            #. The path of the found resource
            #. The .ini file

        Returns
        -------
        List[:class:`IniResource`]
            The constructed resources
        """
        ...
        for part in ifTemplate:
            ...
            currentPaths = part.get(IniKeywords.Filename.value, default = None)
            if (not currentPaths):
                continue
            
            # only get the latest file within the part
            currentPath = currentPaths[-1]
            if (currentPath in result):
                continue

            result[currentPath] = makeRes(currentPath, self)
    ...
    def fix(self, keepBackup: bool = True, fixOnly: bool = False, hideOrig: bool = False) -> Union[str, Dict[Union[str, int], str]]:
        ...
        fixer.clear()
        result = fixer.fix(keepBackup = keepBackup, fixOnly = fixOnly, hideOrig = hideOrig)
