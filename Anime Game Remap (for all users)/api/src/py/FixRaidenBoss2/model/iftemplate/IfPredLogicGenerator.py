...
class IfPredLogicGenerator():
    ...
    @classmethod
    def generate(cls, parseTree: ParseTree, vars: Dict[str, SymbolType], simplify: bool = True) -> Optional[Union[SympBooleanType, bool]]:
        """
        ...
        simplify: :class:`bool`
            Whether the simplify the resultant logic query :raw-html:`<br />` :raw-html:`<br />`
        ...
        generationFuncs = {
            ...
            "eq": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Eq(logic1, logic2)),
            "ne": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Ne(logic1, logic2)),
            "gt": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Gt(logic1, logic2)),
            "ge": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Ge(logic1, logic2)),
            "lt": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Lt(logic1, logic2)),
            "le": lambda node, childrenLogic, vars: cls.generateCompareBinOp(childrenLogic, lambda logic1, logic2: sympy.Le(logic1, logic2)),
            ...
            "nterm reduce": lambda node, childrenLogic, vars: childrenLogic[0],
        ...
        if (result is not None and not isinstance(result, bool) and simplify):
    ...
    def generateVariable(cls, node: ParseNode, vars: Dict[str, SymbolType]) -> SymbolType:
        ...
        variableName = node.token.val
        variableNameKey = variableName[1:]
        result = vars.get(variableNameKey)

        if (result is None):
            sympy = GlobalPackageManager.get(PackageModules.Sympy.value)
            result = sympy.Symbol(f"{variableName}$")
            vars[variableNameKey] = result
    ...
    @classmethod
    def generateCompareBinOp(cls, childrenLogic: List[Union[SympBooleanType, int, float, bool]], 
                             operator: Callable[[Union[SympBooleanType, int, float, bool], Union[SympBooleanType, int, float, bool]], Union[SympBooleanType, int, float, bool]]) -> Optional[Union[SympBooleanType, int, float, bool]]:
        """
        Generates the result for a comparison binary operator
        ...
        operator: Callable[[Union[`sympy.Boolean`_, :class:`bool`], Union[`sympy.Boolean`_, :class:`bool`]], `sympy.Boolean`_]
            The comparison binary operator
        ...
        for i in range(childrenLen):
            ...
            if (isinstance(childLogic, bool)):
                childLogic = int(childLogic)
            elif (not isinstance(childLogic, int) and not isinstance(childLogic, float)):
                if (childLogic == False):
                    childLogic = 0
                elif (childLogic == True):
                    childLogic = 1

            if (i == 0):
                result = childLogic if (childrenLen > 1) else (childLogic != 0)
                continue
