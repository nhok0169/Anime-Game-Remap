    ...
    * If there are any further ambiguity that keyword searching cannot solve, will perform any needed post-processing on the line (eg. regex matching). 
    ...
    def classify(self, iniTxt: Union[str, List[str]], checkIsMod: bool = True, checkIsFixed: bool = True) -> IniClassifyStatsOld:
        ...
        for line in iniTxt:
            ...
            if (not modFound and stats.modType is not None):
                modFound = True
    ...
    def _addTransition(self, srcStateId: Hashable, transition: str, destStateId: Hashable, transitionVal: Union[Optional[ModType], ModTypeBuilder, ModTypes, IniClsAction, Callable[[IniClsActionArgs], Any]]):
        """
        ...
        transitionVal: Union[Optional[:class:`ModType`], :class:`ModTypeBuilder`, :class:`ModTypes`, :class:`IniClsAction`, Callable[[:class:`IniActionArgs`], Any]]
    ...
    def _transition(self, stats: IniClassifyStatsOld, line: str, keyword: str, keywordInd: int = -1, keywordEndInd: int = -1, keywordVals: Optional[IniClsTransitionVals] = None):
        ...
        isModType = isinstance(action, ModType) or isinstance(action, ModTypeBuilder) or isinstance(action, ModTypes)
