...
##### Script
class GIMIOldParser(BaseIniParser):
    """
    ...
    def _getSectionRoots(self):
        ...
        if (not GlobalClassifiers.IniSectionTargets.value.isSetup):
            GlobalClassifiers.IniSectionTargets.value.setup({
        ...
        for sectionName in self._iniFile.sectionIfTemplates:
            ...
            sectionKeySearch = GlobalClassifiers.IniSectionTargets.value.dfa.getAll(cleanedSectionName)
    ...
    def _getBufDownloads(self, sectionGraph: IniSectionGraph, bufKey: str) -> bool:
        ...
        for reg in downloads:
            result = set()
            sectionMissingParts = sectionGraph.rootsGetKeyMissingParts(reg)
    ...
    def getDownloads(self, downloadMode: Optional[DownloadMode] = None):
        ...
        if (hasBufDownloads):
            sectionMissingParts = self.ibCommandsGraph.rootsGetKeyMissingParts(IniKeywords.Handling.value)
    ...
    def _addBufDownloads(self, bufKey: str, modTypeName: str, modType: Optional["ModType"] = None):
        ...
        for reg in bufDownloadParts:
            ...
            ifTemplate = self._makeDownloadResourceIfTemplate(downloadData.name, modTypeName, "", downloadData.download.filename, sectionName = sectionName, downloadKvps = downloadData.resourceKVPs)
